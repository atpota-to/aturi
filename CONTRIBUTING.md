...
- **`composeIntent`** is optional, and only for clients that handle [Bluesky-style intent links](https://docs.bsky.app/docs/advanced-guides/intent-links) — a URL that opens the composer. Declare `{ url: 'https://yourapp.com/intent/compose', textParam: 'text' }` when your route reads the pre-filled text, and drop `textParam` when it opens the composer but ignores it. Omit the field entirely otherwise: consumers read it as "this client can be handed a share link", and a route that 404s is worse than no entry.
