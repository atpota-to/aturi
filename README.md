...
- **Local-first.** No telemetry, no background network calls, and no account unless you want one — signing in is optional, read-only, and used only to import the waypoint setup you already have on the site. The extension otherwise talks only to public atproto services, and only when you open the popup or hit Inspect. Preferences live in your browser's local storage. See [`extension/PRIVACY.txt`](extension/PRIVACY.txt) or [aturi.to/extension/privacy](https://aturi.to/extension/privacy).
