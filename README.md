...
- **Import your published preferences.** Already answered the questions on aturi.to and published them? Options → Defaults takes your handle and pulls the `to.aturi.actor.preferredClients` record in, mapping its rules onto the redirect favorites. One fetch, on demand, when you ask for it.
- **Local-first.** No account, no telemetry, no background network calls. The extension only talks to public atproto services when you open the popup, hit Inspect, or run that import. Preferences live in your browser's local storage. See [`extension/PRIVACY.txt`](extension/PRIVACY.txt) or [aturi.to/extension/privacy](https://aturi.to/extension/privacy).
...
Writing rules by hand means picking a scope before you've decided anything, which is a poor first question. [aturi.to/welcome](https://aturi.to/welcome) asks three concrete ones instead: which client you read Bluesky in, which reader you use for publications, which explorer you want for raw records. Each answer shows the URL it produces, so the setting is visible rather than described, and each option carries a preview link pointed at your own profile so you can look at a reader before choosing it. Every question takes a pass as readily as an answer, answers save as you make them, and the whole thing works signed out with the answers in localStorage until you sign in. It closes on a palette tray, which is the one step that changes the page under you as you click it.

You can also skip the questions entirely. Open a record in a client from any aturi.to link and Aturi offers to make that client your default for records of that kind, which is one click and no wizard. Keep choosing the same client over your current default and it offers to switch. Those counts stay in this browser and are never written to your PDS or sent anywhere; only the preference you agree to leaves the device.
