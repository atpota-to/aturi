...
Runs on macOS or Linux (Swift 5.9 or newer; developed against Swift 6.2). The suite covers the catalog (every waypoint's URL for every record type, pinned as literals), both parser directions, identity resolution, every client against canned responses, preferences round-trips through the PDS record shape, the OAuth handshake against a fake key and transport, and the view models against fake transports. A handful of tests also hit the live network (plc.directory, the public AppView, Constellation, UFOs, Jetstream, the PDS of a public account) and skip rather than fail when it is unreachable.
...
- `AturiCore` compiles and its whole test suite passes under Swift 6.2 on Linux, live-network tests included. That is the catalog, parsers, clients, preferences, OAuth protocol pieces and view models.
- `Aturi.xcodeproj` is generated by XcodeGen from `project.yml` and opens in Xcode; the plists and entitlements it produced are checked in.
- The SwiftUI app target and the share extension were parse-checked (`swiftc -parse`), cross-checked symbol by symbol against the package, and reviewed by independent passes for iOS 17 API correctness, but they have not been compiled by Xcode. Expect the first Xcode build to surface some type errors; the `ios` CI job exists so that a pull request shows them.
- The OAuth sign-in has been exercised end to end only against a fake transport and a fake key, plus live discovery against bsky.social. It has not completed a real login.

## Out of scope for this port

Server-side surfaces have no phone equivalent and were not ported: the MCP server, the Resolve API, OG image rendering, the markdown content negotiation, the sitemap. Permissioned spaces (`/explore/…/space/…`) and the feedback board depend on the space credential flow and are not included. Marketing and docs pages are links out to the site.
